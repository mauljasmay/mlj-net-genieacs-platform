...
Regenerates install.sh and setup.sh with guaranteed Unix LF line endings.
This prevents CRLF corruption from Git on Windows/Mac.

Usage:
    python3 generate-scripts.py

Output:
    install.sh - System installer (11 steps, includes PHP 8.2)
    setup.sh   - App configuration (10 steps, includes PHP)
"""
...
if command -v php8.2 &>/dev/null; then
    ...
    DEBIAN_FRONTEND=noninteractive apt-get install -y -qq \
        php8.2 php8.2-fpm php8.2-sqlite3 php8.2-mbstring \
        php8.2-curl php8.2-xml php8.2-zip php8.2-intl \
        php8.2-readline php8.2-opcache php8.2-redis php8.2-gd \
        > /dev/null 2>&1 || true
...
if command -v node &>/dev/null; then log_info "Node.js: $(node --version)"; else log_error "Node.js: NOT FOUND"; INSTALL_OK=false; fi
if command -v npm &>/dev/null; then log_info "npm: v$(npm --version)"; else log_error "npm: NOT FOUND"; INSTALL_OK=false; fi
if command -v bun &>/dev/null; then log_info "Bun: v$(bun --version)"; else log_warn "Bun: NOT FOUND (will retry in setup)"; fi
if command -v php8.2 &>/dev/null; then log_info "PHP: $(php8.2 --version | head -1)"; else log_error "PHP 8.2: NOT FOUND"; INSTALL_OK=false; fi
if systemctl is-active --quiet php8.2-fpm 2>/dev/null; then log_info "PHP-FPM: running"; else log_warn "PHP-FPM: not running (will start in setup)"; fi
if command -v composer &>/dev/null; then log_info "Composer: $(composer --version 2>/dev/null | head -1)"; else log_warn "Composer: NOT FOUND"; fi
if systemctl is-active --quiet mongod 2>/dev/null; then log_info "MongoDB: running on port 27017"; else log_warn "MongoDB: not running"; fi

CWMP_OK=false; NBI_OK=false; FS_OK=false
...
if command -v caddy &>/dev/null; then log_info "Caddy: $(caddy version 2>/dev/null | head -1)"; else log_error "Caddy: NOT FOUND"; INSTALL_OK=false; fi
if command -v pm2 &>/dev/null; then log_info "PM2: v$(pm2 --version 2>/dev/null)"; else log_error "PM2: NOT FOUND"; INSTALL_OK=false; fi
...
if [ -f "$SETUP_PATH" ]; then
    ...
    echo -e "  ${YELLOW}setup.sh not found.${NC}"
    echo -e "  Please run: sudo bash setup.sh"
fi
'''

def write_script(filepath, content):
    """Write a shell script with Unix LF line endings."""
    with open(filepath, 'w', newline='\n', encoding='utf-8') as f:
        f.write(content)
    os.chmod(filepath, 0o755)
    lines = content.count('\n')
    print(f"[OK] {os.path.basename(filepath)} generated ({lines} lines)")

def main():
    print("MLJ NET - Generating shell scripts with LF line endings...")

    install_path = os.path.join(SCRIPT_DIR, 'install.sh')
    setup_path = os.path.join(SCRIPT_DIR, 'setup.sh')

    # Read current setup.sh if it exists to use it
    if os.path.exists(setup_path):
        with open(setup_path, 'r', encoding='utf-8') as f:
            setup_content = f.read()
        write_script(setup_path, setup_content)
    else:
        print("[WARN] setup.sh not found, only regenerating install.sh")

    write_script(install_path, INSTALL_SH)
    print("\nDone! Scripts ready to run.")
